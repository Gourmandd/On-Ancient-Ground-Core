...
public class BuiltinItemModels extends ItemModelProvider
{
    ...
    protected void registerModels()
    {
        ...
        Stream.of(SpectrumWood.values()).forEach(woodType ->
        {
            ...
            withExistingParent(getItemModelString(block.getId()), getBlockModelLocation(locationOpened)).override().predicate(ResourceLocation.fromNamespaceAndPath(TerraFirmaCraft.MOD_ID, "sealed"), 1.0f).model(modelSealed);
        });
        ...
        for (OreDeposit ore : OreDeposit.values())
        {
            for (CoreRocks rock : CoreRocks.values())
            {
                if (rock.hasOres())
                {
                    ...
                    this.getBuilder("%s:item/pan/%s/%s_full".formatted(AncientGroundCore.MOD_ID, oreId, rockId)).parent(new ModelFile.UncheckedModelFile("tfc:item/pan/full")).texture("material", texture);

                    this.getBuilder("%s:item/pan/%s/%s_half".formatted(AncientGroundCore.MOD_ID, oreId, rockId)).parent(new ModelFile.UncheckedModelFile("tfc:item/pan/half")).texture("material", texture);
                }
            }
        }
    ...
    private void simpleBucket(DeferredHolder<Item, Item> holder)
    {
        ...
        {
            withExistingParent(getItemModelString(holder.get()), ResourceLocation.fromNamespaceAndPath(NeoForgeVersion.MOD_ID, "item/bucket")).customLoader(DynamicFluidContainerModelBuilder::begin).fluid(bucket.content);
        }
    }
}
