...
public class BuiltinBlockLootTables extends BlockLootSubProvider
{
    ...
    {
        return this.createSilkTouchOrShearsDispatchTable(CoreBlocks.FRUIT_TREE_LEAVES.get(tree).get(), this.applyExplosionCondition(CoreBlocks.FRUIT_TREE_LEAVES.get(tree).get(), LootItem.lootTableItem(Items.STICK).when(LootItemBlockStatePropertyCondition.hasBlockStateProperties(CoreBlocks.FRUIT_TREE_LEAVES.get(tree).get()).setProperties(StatePropertiesPredicate.Builder.properties().hasProperty(TFCBlockStateProperties.LIFECYCLE, Lifecycle.FRUITING)))));
    }

    //gemstones
    protected LootTable.Builder createClusterTable(Block block, Item item, float amount)
    {
        return this.createSilkTouchDispatchTable(block, LootItem.lootTableItem(item).apply(SetItemCountFunction.setCount(ConstantValue.exactly(amount))).apply(ApplyBonusCount.addOreBonusCount(this.registries.holderOrThrow(Enchantments.FORTUNE))).when(MatchTool.toolMatches(ItemPredicate.Builder.item().of(ItemTags.CLUSTER_MAX_HARVESTABLES))).otherwise(this.applyExplosionDecay(block, LootItem.lootTableItem(item).apply(SetItemCountFunction.setCount(ConstantValue.exactly(2.0F))))));
    }
}
