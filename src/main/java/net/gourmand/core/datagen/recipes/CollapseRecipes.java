...
public interface CollapseRecipes extends Recipes
{
    ...
    private void addRock(CoreRocks rockType)
    {
        ...
        {
            collapse(BlockIngredient.of(CategoryUtil.CoreRock.TO_RAW_BLOCK.get(rockType).value(), BLOCK_MAP.get(Rock.BlockType.HARDENED).get(), BLOCK_MAP.get(Rock.BlockType.SMOOTH).get(), BLOCK_MAP.get(Rock.BlockType.CRACKED_BRICKS).get()), BLOCK_MAP.get(Rock.BlockType.COBBLE).get().defaultBlockState());
        }
        else
        {
            collapse(BlockIngredient.of(CategoryUtil.CoreRock.TO_RAW_BLOCK.get(rockType).value(), BLOCK_MAP.get(Rock.BlockType.HARDENED).get()), BLOCK_MAP.get(Rock.BlockType.COBBLE).get().defaultBlockState());
        }
    }
