...
public interface WeldingRecipes extends Recipes
{
    ...
    {
        add(new WeldingRecipe(ingredientOf(ingot1, ItemType.INGOT), ingredientOf(ingot2, ItemType.INGOT), ingotOut.tier() - 1, ItemStackProvider.of(CoreItems.METAL_ITEMS.get(ingotOut).get(ItemType.INGOT).get()), IGNORE));
    }
    ...
    {
        add(new WeldingRecipe(ingredientOf(metal, input1), ingredientOf(metal, input2), metal.tier() - 1, ItemStackProvider.of(CoreItems.METAL_ITEMS.get(metal).get(output).get()), behavior));
    }
}
