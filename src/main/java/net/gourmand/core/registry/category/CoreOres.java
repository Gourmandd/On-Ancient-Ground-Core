...
public enum CoreOres implements RegistryOre
{
    ...
    SHIMMERSTONE(Type.NORMAL, SpectrumOreType.SHIMMERSTONE), AZURITE(Type.NORMAL, SpectrumOreType.AZURITE), STRATINE(Type.NORMAL, SpectrumOreType.STRATINE), PALTAERIA(Type.NORMAL, SpectrumOreType.PALTAERIA), MALACHITE(Type.NORMAL, SpectrumOreType.MALACHITE), GALENA(Type.GRADED, CoreMetals.MetalType.LEAD), METEORIC_IRON(Type.ITEM_ONLY), QUARTZ(Type.ITEM_ONLY), ANTHRACITE(Type.ITEM_ONLY), BAUXITE(Type.ITEM_ONLY);
    ...
    public Block create(RegistryRock rock)
    {
        ...
        {
            case SpectrumOreType.SHIMMERSTONE ->
                    block = new ShimmerstoneOreBlock(UniformInt.of(2, 4), properties, SpectrumAdvancements.REVEAL_SHIMMERSTONE, getRockBlockState(rock));
            case SpectrumOreType.AZURITE ->
                    block = new AzuriteOreBlock(UniformInt.of(4, 7), properties, SpectrumAdvancements.REVEAL_AZURITE, getRockBlockState(rock));
            case SpectrumOreType.STRATINE ->
                    block = new CloakedOreBlock(UniformInt.of(3, 5), properties, SpectrumAdvancements.REVEAL_STRATINE, getRockBlockState(rock));
            case SpectrumOreType.PALTAERIA ->
                    block = new CloakedOreBlock(UniformInt.of(2, 4), properties, SpectrumAdvancements.REVEAL_PALTAERIA, getRockBlockState(rock));
            case SpectrumOreType.MALACHITE ->
                    block = new CloakedOreBlock(UniformInt.of(7, 11), properties, SpectrumAdvancements.REVEAL_MALACHITE, getRockBlockState(rock));
            case SpectrumOreType.NONE -> block = new Block(properties);
            default ->
                    throw new AssertionError("spectrumOreType of CoreOre did not match any PastelOreType enum entry");
        }
        return block;
    }
    ...
    public Item getPastelOre()
    {
        ...
        switch (spectrumOreType)
        {
            ...
            case SpectrumOreType.NONE ->
                    throw new AssertionError("spectrumOre of CoreOre is NONE, only use the method if you are sure spectrumOreType isnt NONE");
