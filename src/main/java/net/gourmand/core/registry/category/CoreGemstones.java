...
public enum CoreGemstones implements GemstoneColor, StringRepresentable
{
    ...
    AQUAMARINE(InkColors.BLUE, 5), SPINEL(InkColors.RED, 6), JADE(InkColors.GREEN, 7);
    ...
    public enum GemstoneBlocks implements StringRepresentable
    {
        ...
        BLOCK((gem) -> new SpectrumGemstoneBlock(gemstoneBlockSettings(gem), SpectrumSoundEvents.BLOCK_ONYX_BLOCK_HIT, SpectrumSoundEvents.BLOCK_ONYX_BLOCK_CHIME)), PILLAR((gem) -> new Block(gemstoneBlockSettings(gem))), POWDER_BLOCK((gem) -> new ColoredFallingBlock(new ColorRGBA(gem.getDyeColor().getFireworkColor()), BlockBehaviour.Properties.ofFullCopy(Blocks.SAND).mapColor(gem.getMapColor()))), CLUSTER((gem) -> new SpectrumClusterBlock(clusterBlockSettings(gem), SpectrumClusterBlock.GrowthStage.CLUSTER), true), LARGE_CLUSTER((gem) -> new SpectrumClusterBlock(clusterBlockSettings(gem), SpectrumClusterBlock.GrowthStage.LARGE), true), MEDIUM_CLUSTER((gem) -> new SpectrumClusterBlock(clusterBlockSettings(gem), SpectrumClusterBlock.GrowthStage.MEDIUM), true), SMALL_CLUSTER((gem) -> new SpectrumClusterBlock(clusterBlockSettings(gem), SpectrumClusterBlock.GrowthStage.SMALL), true), BUDDING_BLOCK((gem) -> new SpectrumBuddingBlock(gemstoneBlockSettings(gem).pushReaction(PushReaction.DESTROY).randomTicks(), CoreBlocks.GEMSTONE_BLOCKS.get(gem).get(SMALL_CLUSTER).get(), CoreBlocks.GEMSTONE_BLOCKS.get(gem).get(MEDIUM_CLUSTER).get(), CoreBlocks.GEMSTONE_BLOCKS.get(gem).get(LARGE_CLUSTER).get(), CoreBlocks.GEMSTONE_BLOCKS.get(gem).get(CLUSTER).get(), SpectrumSoundEvents.BLOCK_ONYX_BLOCK_HIT, SpectrumSoundEvents.BLOCK_ONYX_BLOCK_CHIME));
    ...
    {
        SHARD((gem) -> new Item(new Item.Properties().rarity(Rarity.UNCOMMON))), POWDER((gem) -> new Item(new Item.Properties().rarity(Rarity.UNCOMMON)));
