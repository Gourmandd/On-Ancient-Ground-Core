...
public class CreativeTabs
{
    ...
    private static void fillOres(CreativeModeTab.ItemDisplayParameters parameters, CreativeModeTab.Output out)
    {
        ...
        for (CoreOres ore : CoreOres.values())
        {
            if (!(ore.isGraded() || ore.hasSpectrumOreType()))
                accept(out, CoreItems.ORES, ore);
        }
    ...
    private static void fillWood(CreativeModeTab.ItemDisplayParameters parameters, CreativeModeTab.Output out)
    {
        ...
        CoreBlocks.SPECTRUM_WOOD_SHUTTERS.forEach((wood, block) -> out.accept(block.get()));
        CoreBlocks.TFC_WOOD_SHUTTERS.forEach((wood, block) -> out.accept(block.get()));
        CoreBlocks.AFC_WOOD_SHUTTERS.forEach((wood, block) -> out.accept(block.get()));
    }
